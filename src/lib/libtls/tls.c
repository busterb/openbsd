...
tls_configure_ssl(struct tls *ctx, SSL_CTX *ssl_ctx)
{
	...
	if (tls_config_is_dtls(ctx->config)) {
		...
		SSL_CTX_set_cookie_generate_cb(ctx->ssl_ctx,
		    tls_generate_cookie);
...
tls_generate_cookie(SSL *ssl, unsigned char *cookie, unsigned int *cookie_len)
{
	...
	case AF_INET:
		...
		if (HMAC_Update(&hmac, (unsigned char*) &peer.s4.sin_addr,
		    sizeof(peer.s4.sin_addr)) != 1) {
			goto cleanup;
		}
		if (HMAC_Update(&hmac, (unsigned char*) &peer.s4.sin_port,
		    sizeof(peer.s4.sin_port)) != 1) {
			goto cleanup;
		}
	break;
	...
	case AF_INET6:
		...
		if (HMAC_Update(&hmac, (unsigned char*) &peer.s6.sin6_addr,
		    sizeof(peer.s6.sin6_addr)) != 1) {
			goto cleanup;
		}
		if (HMAC_Update(&hmac, (unsigned char*) &peer.s6.sin6_port,
		    sizeof(peer.s6.sin6_port)) != 1) {
			goto cleanup;
		}
		break;
	}
